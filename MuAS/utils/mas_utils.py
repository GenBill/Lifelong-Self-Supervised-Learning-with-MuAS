...
def init_reg_params(model, device, freeze_layers = []):
    """
    Input:
...
def init_reg_params_across_tasks(model, device, freeze_layers = []):
    """
    Input:
    1) model: A reference to the model that is being trained
    2) use_gpu: Set the flag to True if the model is to be trained on the GPU
...
def consolidate_reg_params(model, device):
    """
    Input:
    1) model: A reference to the model that is being trained
...
def compute_omega_grads_norm(model, dataloader, optimizer, device):
    ...
    for data in dataloader:
        ...
        optimizer.step(model.reg_params, index, labels.size(0), device)
        del labels
